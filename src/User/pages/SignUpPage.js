...
const SignUpPage = () => {
  ...
  const { register, loading, googleAuth } = useAuth();
  ...
  const validateForm = () => {
    ...
    if (!formData.name) {  // Add name validation
    ...
    if (result.success) {
      ...
      navigate('/verify-email', { 
        state: { 
          ...
          message: result.message || 'Account created successfully! Please check your email for verification code.'
        }
      });
    } else {
      if (result.errors) {
